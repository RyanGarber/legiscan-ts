...
import { asName, Progress, SponsorType, State } from "./enums.js";
/**
...
export class LegiscanClient {
    ...
    async request(op, params = {}) {
        ...
        if (response.status == "ERROR") {
            throw new Error(`Legiscan API rejected request: ${response.alert?.message ?? "no reason given"}`);
        }
        return response;
    }
    /*
    ...
    */
     ...
     */
    async getMasterList({ state, session, }) {
    ...
    async getSponsoredList({ id }) {
        ...
        const sessionNames = Object.fromEntries(sessions.map((s) => [
            s.session_id,
            s.session_name,
        ]));
     ...
     */
    async getSearch({ query, state, year, }) {
        const all = [];
     ...
     * @param year Year where 1=all, 2=current, 3=recent, 4=prior, >1900=exact
     */
    async *getSearchAsync({ query, state, year, }) {
        let page = 1;
        while (true) {
